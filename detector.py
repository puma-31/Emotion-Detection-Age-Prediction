...
def _resolve_model_path(preferred_path: str, fallback_path: str) -> str:
    ...
    if fb.exists():
        return str(fb)
    # return preferred path so the error from load_model is informative
    return str(p)


def load_models():
    repo_root = Path(__file__).resolve().parents[1]
    # prefer curated models/ folder at repo root
    preferred_emotion = repo_root / 'models' / 'best_model.keras'
    fallback_emotion = Path(__file__).resolve().parent / 'best_model.keras'

    preferred_age = repo_root / 'models' / 'best_age_model.keras'
    fallback_age = Path(__file__).resolve().parent / 'best_age_model.keras'
...
def predict_face(face_bgr, emotion_model, age_model):
...
def main():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            out = predict_face(face_crop, emotion_model, age_model)
            emotion = out['emotion']
            age = out['age']
            probs = out['probs']
            ...
            cv2.putText(frame, label, (x1, label_y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)

            for i, (emo, prob) in enumerate(zip(EMOTION_LABELS, probs)):
                bar_x = 10
                bar_y = 20 + i * 22
